...
export function path() {
  ...
  for (let i = 0; i < process.cwd().split(Path.sep).length; i++) {
    ...
    paths.push(Path.resolve(Path.join(process.cwd(), ...dots, 'node_modules', 'ipfs')));
  }
  const resourcePath = paths.find(path => fs.existsSync(path));
  if (!resourcePath) {
    throw new Error(`Could not find ipfs module in paths: \n${ paths.join('\n') }`);
  }
  const pkg = JSON.parse(fs.readFileSync(resourcePath + Path.sep + 'package.json', { encoding: 'utf-8' }));
