...
(1) For CID version 0 the multibase must be base58btc and no prefix is used. For Cid version 1 the multibase prefix is included.`,
  ...
  async handler({
    ...
    for await (const data of input) {
      ...
      let cid$1 = cid.CID.parse(str);
      if (cidVersion != null && cid$1.version !== cidVersion) {
        if (cidVersion === 0) {
          cid$1 = cid$1.toV0();
      ...
      print(formatStr.replace(/%([a-zA-Z%])/g, replacer(cid$1, cidBase, ipfs)));
    }
  }
};
...
function findBase(str, ipfs) {
  if (cid.CID.parse(str).version === 0) {
    str = `z${ str }`;
  }
