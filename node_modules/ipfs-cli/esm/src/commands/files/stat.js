...
Mtime: <mtime>`,
  ...
  async handler({
    ...
    print(format.replace('<hash>', stats.cid.toString(base.encoder)).replace('<size>', `${ stats.size }`).replace('<cumulsize>', `${ stats.cumulativeSize }`).replace('<childs>', `${ stats.blocks }`).replace('<type>', stats.type).replace('<mode>', stats.mode ? formatMode(stats.mode, stats.type === 'directory') : '').replace('<mtime>', stats.mtime ? formatMtime(stats.mtime) : ''));
  }
};