...
import { stripControlCharacters } from '../utils.js';
...
export default {
  ...
  async handler({
    ...
    await pipe(ipfs.get(ipfsPath, {
      ...
      compress,
      compressionLevel
    }), extract(), async function extractTarball(source) {
      for await (const {header, body} of source) {
        ...
        if (header.type === 'file') {
          ...
          await pipe(body, source => map(source, buf => buf.slice()), toIterable.sink(fs.createWriteStream(outputPath)));
