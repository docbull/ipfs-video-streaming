  ...
  async handler(request, h) {
    ...
    return streamResponse(request, h, () => {
      return async function* () {
        for await (const event of ipfs.dht.findPeer(peerId, { signal: anySignal(signals) })) {
...
export const findProvsResource = {
  ...
  async handler(request, h) {
    ...
    return streamResponse(request, h, () => {
      return async function* () {
        for await (const event of ipfs.dht.findProvs(cid, { signal: anySignal(signals) })) {
...
export const getResource = {
  ...
  async handler(request, h) {
    ...
    return streamResponse(request, h, () => {
      return async function* () {
        for await (const event of ipfs.dht.get(key, { signal: anySignal(signals) })) {
          yield mapQueryEvent(id.id, event);
        }
        if (timeoutController) {
          timeoutController.clear();
...
export const provideResource = {
  ...
  async handler(request, h) {
    ...
    return streamResponse(request, h, () => {
      return async function* () {
        for await (const event of ipfs.dht.provide(cid, { signal: anySignal(signals) })) {
          yield mapQueryEvent(id.id, event);
        }
        if (timeoutController) {
          timeoutController.clear();
...
export const putResource = {
  ...
  async handler(request, h) {
    ...
    return streamResponse(request, h, () => {
      return async function* () {
        for await (const event of ipfs.dht.put(key, value, { signal: anySignal(signals) })) {
          yield mapQueryEvent(id.id, event);
        }
        if (timeoutController) {
          timeoutController.clear();
...
export const queryResource = {
  ...
  async handler(request, h) {
    ...
    return streamResponse(request, h, () => {
      return async function* () {
        for await (const event of ipfs.dht.query(key, { signal: anySignal(signals) })) {
          yield mapQueryEvent(id.id, event);
        }
        if (timeoutController) {
          timeoutController.clear();
