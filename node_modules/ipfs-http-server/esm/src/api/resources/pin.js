...
export const lsResource = {
  ...
  async handler(request, h) {
    ...
    if (!stream) {
      const res = await pipe(source, function collectKeys(source) {
        const init = { Keys: {} };
    ...
    return streamResponse(request, h, () => pipe(source, async function* transform(source) {
...
export const addResource = {
  ...
  async handler(request, h) {
    ...
    let result;
    try {
      result = await all(ipfs.pin.addAll(cids.map(cid => ({
        cid,
        recursive,
        metadata
      })), {
...
export const rmResource = {
  ...
  async handler(request, h) {
    ...
    let result;
    try {
      result = await all(ipfs.pin.rmAll(cids.map(cid => ({
        cid,
        recursive
      })), {
        signal,
