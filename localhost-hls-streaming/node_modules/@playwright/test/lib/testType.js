...
class TestTypeImpl {
  ...
  _ensureCurrentSuite(location, title) {
    ...
    if (!suite) {
      throw (0, _util.errorWithLocation)(location, [`Playwright Test did not expect ${title} to be called here.`, `Most common reasons include:`, `- You are calling ${title} in a configuration file.`, `- You are calling ${title} in a file that is imported by the configuration file.`, `- You have two different versions of @playwright/test. This usually happens`, `  when one of the dependenices in your package.json depends on @playwright/test.`].join('\n'));
    }
  ...
  _configure(location, options) {
    ...
    for (let parent = suite.parent; parent; parent = parent.parent) {
      ...
      if (parent._parallelMode === 'parallel' && suite._parallelMode === 'serial') throw (0, _util.errorWithLocation)(location, 'describe.serial cannot be nested inside describe.parallel');
    }
  }
