...
class Runner {
  ...
  async _runFiles(list, filesByProject, testFileReFilters) {
    ...
    for (const [project, files] of filesByProject) {
      const grepMatcher = (0, _util2.createTitleMatcher)(project.config.grep);
      const grepInvertMatcher = project.config.grepInvert ? (0, _util2.createTitleMatcher)(project.config.grepInvert) : null;
      const projectSuite = new _test.Suite(project.config.name);
      projectSuite._projectConfig = project.config;
      if (project.config.fullyParallel) projectSuite._parallelMode = 'parallel';
  ...
  async _performGlobalSetup(config) {
    ...
    await this._runAndReportError(async () => {
      ...
      webServer = config.webServer ? await _webServer2.WebServer.create(config.webServer, this._reporter) : undefined;
