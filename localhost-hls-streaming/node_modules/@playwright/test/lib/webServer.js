...
class WebServer {
  ...
  static async create(config, reporter) {
  ...
  async _startProcess() {
    ...
    if (isAlreadyAvailable) {
      ...
      throw new Error(`${(_this$config$url = this.config.url) !== null && _this$config$url !== void 0 ? _this$config$url : `http://localhost:${this.config.port}`} is already used, make sure that nothing is running on the port/url or set reuseExistingServer:true in config.webServer.`);
    }
    ...
    launchedProcess.stderr.on('data', line => {
      var _this$reporter$onStdE, _this$reporter;

      return (_this$reporter$onStdE = (_this$reporter = this.reporter).onStdErr) === null || _this$reporter$onStdE === void 0 ? void 0 : _this$reporter$onStdE.call(_this$reporter, '[WebServer] ' + line.toString());
    });
    launchedProcess.stdout.on('data', line => {
      var _this$reporter$onStdO, _this$reporter2;

      if (debugWebServer.enabled) (_this$reporter$onStdO = (_this$reporter2 = this.reporter).onStdOut) === null || _this$reporter$onStdO === void 0 ? void 0 : _this$reporter$onStdO.call(_this$reporter2, '[WebServer] ' + line.toString());
    });
  }
