...
class WorkerRunner extends _events.EventEmitter {
  ...
  async _teardownScopes() {
    ...
    if (result.timedOut) this._fatalErrors.push({
  ...
  async _runSuite(suite, annotations) {
    ...
    for (const beforeAllModifier of suite._modifiers) {
      ...
      if (result.timedOut) {
        this._fatalErrors.push((0, _util2.serializeError)(new Error(`Timeout of ${this._project.config.timeout}ms exceeded while running ${beforeAllModifier.type} modifier\n    at ${(0, _util2.formatLocation)(beforeAllModifier.location)}`)));
