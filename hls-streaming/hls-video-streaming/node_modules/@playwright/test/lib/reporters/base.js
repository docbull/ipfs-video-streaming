...
class BaseReporter {
  ...
  onError(error) {
    console.log('\n' + formatError(this.config, error, _safe.default.enabled).message);
  }
...
function formatFailure(config, test, options = {}) {
  ...
  for (const result of test.results) {
    const resultLines = [];
    const errors = formatResultFailure(config, test, result, '    ', _safe.default.enabled);
    if (!errors.length) continue;
    const retryLines = [];

    if (result.retry) {
      retryLines.push('');
      retryLines.push(_safe.default.gray(pad(`    Retry #${result.retry}`, '-')));
    }
...
function formatResultFailure(config, test, result, initialIndent, highlightCode) {
  ...
  if (result.status === 'passed' && test.expectedStatus === 'failed') {
    errorDetails.push({
      message: indent(_safe.default.red(`Expected to fail, but passed.`), initialIndent)
    });
  }
...
function fitToWidth(line, width, suffix) {
