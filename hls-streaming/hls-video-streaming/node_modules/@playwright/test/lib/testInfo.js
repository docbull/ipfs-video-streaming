    ...
    try {
      ...
      if (this.status === 'passed') {
        ...
        if (this._test._type === 'test') {
          this.errors.push({
            message: _safe.default.red(`Timeout of ${this.timeout}ms exceeded.`)
          });
        } else {
          // Include location for the hook to distinguish between multiple hooks.
          const message = _safe.default.red(`Timeout of ${this.timeout}ms exceeded in ${this._test._type} hook.`);

          this.errors.push({
            message: message,
            stack: message + `\n    at ${(0, _util.formatLocation)(this._test.location)}.`
          });
        }
      }
    }
