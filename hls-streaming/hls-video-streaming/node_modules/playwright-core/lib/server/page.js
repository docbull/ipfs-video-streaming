...
class Page extends _instrumentation.SdkObject {
  ...
  async expectScreenshot(metadata, options = {}) {
    ...
    const comparator = _comparators.mimeTypeToComparator['image/png'];
    const controller = new _progress.ProgressController(metadata, this);
    const isGeneratingNewScreenshot = !options.expected;
    if (isGeneratingNewScreenshot && options.isNot) return {
    ...
    return controller.run(async progress => {
      ...
      while (true) {
        ...
        if (comparatorResult !== undefined && !!comparatorResult === !!options.isNot) break;
