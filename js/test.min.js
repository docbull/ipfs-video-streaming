(function (root, factory) {
    (typeof module === 'object' && module.exports) ? module.exports = factory() : root.HlsjsIpfsLoader = factory()}
            ...
            var w=g((p,b)=>{
                ...
                var c=class{
                    ...
                    loadInternal(){
                        ...
                        if(o === "master0.ts") {
                            let testing = "master0.ts"
                            this._abortFlag[0]=!1,v(this.ipfs,this.hash,testing,l,this.debug,this._abortFlag).then(s=>{
                ...
                async function m(n,t,e,i,a){
                    let start=new Date();
                    let o=[],l=0,h=0;
                    for await(let s of e.cat(n,t))
                    if(o.push(s),l+=s.length,a[0]){
                        i("Cancel reading from ipfs");
                        break
                    }

                    // docbull watson. ///////////////////////////
                    let curChunk=n.split('/')[1];
                    let curChunkNum=curChunk.split('.')[0];
                    //console.log("current chunk:", curChunk);
                    let prevChunk=await test();
                    let prevChunkNum=prevChunk.split('.')[0];
                    inlab=curChunk;
                    // for (;;) {
                    //     if (curChunk === "master.m3u8" || prevChunk === "master.m3u8") {
                    //         break;
                    //     }
                    //     if (prevChunkNum === curChunkNum[curChunkNum.length-1]-1) {
                    //         break;
                    //     } else {
                    //         continue;
                    //     }
